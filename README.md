...
- [关键字参考](docs/KEYWORD_REFERENCE.md) - YAML/INI 配置关键字
...
- `step_xx/isomers_failed.xyz`：失败构象（输入结构）集合，注释行包含 `Job/CID/Error`，方便定位与重算。
- `step_xx/work/results.db`：每个 `geom_XXXX` 的状态与 `error/error_details`。
