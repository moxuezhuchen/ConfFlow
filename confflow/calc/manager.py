...
- 结果库路径/备份恢复/输出 isomers.xyz / auto_clean 行为保持一致
"""
...
class ChemTaskManager:
    def __init__(self, settings_file: str, resume_dir: Optional[str] = None):
    ...
    def _ensure_work_dir(self):
        ...
        if backup_dir_cfg and str(backup_dir_cfg).strip():
            ...
            self.backup_dir = os.path.join(os.path.dirname(self.work_dir), "backups")
    ...
    def run(self, input_xyz_file: str) -> None:
        ...
        if failed:
            ...
            failed_file = os.path.join(self.work_dir, "isomers_failed.xyz")
        ...
        out_file = os.path.join(self.work_dir, "isomers.xyz")
        ...
        if self.config.get("auto_clean", "false").lower() == "true":
            console.print()
            console.rule("自动后处理 (Auto Refine)", characters="-")
            # print("\n🧹 自动后处理: 调用 refine 模块")
            try:
                ...
                if "-t" in opts_str:
                    try:
                        ...
                        logger.debug(f"clean_opts -t 解析失败: {e}")
                if "-ewin" in opts_str:
                    try:
                ...
                clean_args = refine.RefineOptions(
                    input_file=out_file,
                    output=os.path.join(os.path.dirname(out_file), "isomers_cleaned.xyz"),
                    threshold=thresh,
                    ewin=ewin,
                    workers=task_cores,
                )
...
def main():
    ...
    parser = argparse.ArgumentParser(
        ...
        epilog="示例: confcalc traj.xyz -s settings.ini",
    )
