#!/usr/bin/env python3
"""Legacy DAG implementation kept for one release cycle of compatibility.

The producer-side execution semantics moved to :mod:`confflow.workflow.dag.explicit`
in v1.4.3. The classes in this module remain importable through
``confflow.workflow.dag`` so existing user code and downstream test suites
keep running, but every access through the parent package triggers a
``DeprecationWarning``. Import them directly from
``confflow.workflow.dag.legacy`` to silence that warning for one release
cycle (v1.4.4 deprecated; earliest removal v1.5.0).
"""
...
__all__ = ["DAGStep", "DAGGraph", "WorkflowDAG"]
...
_CONDITION_PATTERN = re.compile(r"prev\.(\w+)\s*(==|!=|<=|>=|<|>)\s*(.+)")
...
class WorkflowDAG:
    ...
    .. deprecated::
        Use :mod:`confflow.workflow.dag.explicit` for new code. This
        implementation is preserved for one release cycle and will be
        removed no earlier than v1.5.0.
    """
