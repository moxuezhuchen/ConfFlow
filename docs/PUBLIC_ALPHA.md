...
- Release status: v2.0.0 remains the published GitHub Release. The v2.1.0 tag was superseded before publication after its release-only Linux venv-path gate failed. The v2.1.1 workflow is prepared to build and publish wheel/sdist artifacts, checksums, SBOM, release provenance, and build attestation; PyPI publishing remains manual.
...
- PyPI publishing is not automated. The published v2.0.0 release record remains historical; the v2.1.1 GitHub Release workflow automates release artifact publication, artifact provenance, and build attestation once separately authorized. Full SLSA-style hardening remains a separate limitation.
