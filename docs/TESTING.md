...
| `test_config_contract_v2_cli.py` | config/cli | `config contract [--version {1,2,3}]` 默认/显式行为、确定性、非法版本（v3 由 `test_configuration_contract_v3.py` 覆盖） |
...
- `.venv/bin/python -m pytest --collect-only -q`：701 tests collected
- `.venv/bin/python -m pytest -q`：701 passed
- `.venv/bin/python -m pytest tests/ --cov=confflow --cov-report=term-missing --cov-report=xml`：通过，total coverage 85.65%
