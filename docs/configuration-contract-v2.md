...
| C16 | `calc.task`’s choice **order** differs. The consumer fallback lists `opt, opt_freq, sp, freq, ts`; this branch derives the order from `Literal[TaskName]` in `confflow/config/canonical/types.py`, giving `opt, sp, freq, opt_freq, ts`. | The *set* of choices is identical — same five values and labels — and the consumer parser accepts any order with zero diagnostics, so the difference is cosmetic. It is the **only** difference found when all 27 fields × 11 attributes are compared against the shipped fallback manifest (field ids, pointers, editors, labels, groups, levels, `read_only`, defaults, conditions and inheritance all match exactly). | Deliberately not “fixed”: dropdown order is presentation, and ConfFlow owns semantics while JobDesk owns presentation. Deriving from the literal also means a task added to `TaskName` shows up automatically. JobDesk may re-sort if it wants the older order. |
