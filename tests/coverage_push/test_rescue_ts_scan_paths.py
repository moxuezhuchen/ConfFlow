...
def test_rescue_helpers(tmp_path):
    from confflow.calc.rescue import (
...
def test_ts_rescue_scan_no_coords(tmp_path):
    ...
    task_info = {
        ...
        "config": {"itask": 4, "iprog": 1, "ts_bond": "1,2"},
...
def test_ts_rescue_scan_freq_failure(tmp_path):
    ...
    task_info = {
        "job_name": "test",
        "work_dir": str(tmp_path / "work"),
        "config": {"itask": 4, "iprog": 1, "keyword": "freq", "ts_bond": "1,2"},
        "coords": ["H 0 0 0", "H 0 0 1.0"],
    }
...
def _task_info_for_scan(tmp_path, *, keyword: str, config_extra: Optional[Dict] = None, job: str = "job"):
    wd = str(tmp_path)
    os.makedirs(wd, exist_ok=True)
    cfg = {"keyword": keyword, "ts_bond": "1,2"}
    if config_extra:
...
def test_ts_rescue_scan_full_logic(tmp_path):
    ...
    with patch("confflow.calc.rescue.executor._run_calculation_step") as mock_run, \
        patch("confflow.calc.rescue.GaussianPolicy"):
...
def test_ts_rescue_scan_internal_patches_v8_style(tmp_path):
    ...
    with patch("confflow.calc.rescue.executor._run_calculation_step", side_effect=mock_run_step):
        with patch("confflow.calc.rescue.GaussianPolicy"), \
            ...
            patch("confflow.calc.rescue._coords_array_from_xyz_lines", return_value=np.zeros((3, 3))), \
            patch("confflow.calc.rescue.refine.fast_rmsd", return_value=0.01), \
            patch("confflow.calc.rescue._set_bond_length_on_coords", side_effect=lambda c, a1, a2, r: [str(r)]):
...
def test_ts_rescue_scan_freq_failure_with_lowest(tmp_path):
    ...
    def mock_run_step(ts_wd, ts_job, policy, coords, ts_cfg, **kwargs):
        if "scan_0" in ts_job:
            return {"e_low": -1.0, "final_coords": coords}
        if "_rescue" in ts_job and "scan" not in ts_job:
            return {"final_coords": coords, "e_low": -1.0, "num_imag_freqs": 2, "lowest_freq": -100.0}
        return {"e_low": -1.1, "final_coords": coords}
